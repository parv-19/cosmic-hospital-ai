    ...
    <div className="space-y-3">
      {data.map((item) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="w-32 shrink-0 truncate text-xs font-medium text-slate-500 dark:text-slate-300">{item.label}</span>
          ...
          <span className="w-8 shrink-0 text-right text-xs font-semibold text-slate-700 dark:text-slate-100">{item.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-52 w-full overflow-visible" role="img" aria-label="Call trend">
        {[0, 1, 2, 3].map((line) => (
          <line key={line} x1="0" x2={width} y1={28 + line * 44} y2={28 + line * 44} className="stroke-slate-200 dark:stroke-slate-700" strokeWidth="1" />
        ))}
        ...
        {coordinates.map((point) => (
          <g key={`${point.label}-${point.x}`}>
            <circle cx={point.x} cy={point.y} r="5" className="fill-white dark:fill-slate-950" stroke={color} strokeWidth="3" />
            <text x={point.x} y={point.y - 12} textAnchor="middle" className="fill-slate-500 dark:fill-slate-300 text-[19px] font-semibold">
              {point.value}
            </text>
          </g>
        ))}
    ...
    <div className="flex items-center gap-5">
      <div className="relative h-36 w-36 shrink-0">
        <svg viewBox="0 0 42 42" className="h-36 w-36 -rotate-90" role="img" aria-label={centerLabel}>
          <circle cx="21" cy="21" r="15.915" fill="none" className="stroke-slate-200 dark:stroke-slate-800" strokeWidth="6" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-lg font-bold text-slate-800 dark:text-white">{centerValue}</span>
          <span className="text-[10px] font-semibold uppercase text-slate-400 dark:text-slate-500">{centerLabel}</span>
        </div>
      </div>
        ...
        {segments.map((segment) => (
          <div key={segment.label}>
            <div className="flex items-center justify-between gap-3 text-xs">
              ...
              <span className="font-bold text-slate-800 dark:text-white">{total ? `${pct(segment.value, total)}%` : "0%"}</span>
            </div>
...
function InsightRow({ label, value, tone }: { label: string; value: string; tone: "cyan" | "emerald" | "amber" | "rose" }) {
  const toneClass = {
    cyan: "border-cyan-100 bg-cyan-50 text-cyan-700 dark:border-cyan-500/20 dark:bg-cyan-500/12 dark:text-cyan-200",
    emerald: "border-emerald-100 bg-emerald-50 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/12 dark:text-emerald-200",
    amber: "border-amber-100 bg-amber-50 text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/12 dark:text-amber-200",
    rose: "border-rose-100 bg-rose-50 text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/12 dark:text-rose-200",
  }[tone];
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 xl:grid-cols-5 gap-4">
        <StatCard
          ...
          icon={<svg width="20" height="20" className="text-cyan-600 dark:text-cyan-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>}
        />
        <StatCard
          label="Booking Rate"
          ...
          sub="of total calls"
          icon={<svg width="20" height="20" className="text-emerald-600 dark:text-emerald-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>}
        />
        <StatCard
          label="Transfer Rate"
          ...
          sub="of total calls"
          icon={<svg width="20" height="20" className="text-amber-600 dark:text-amber-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="17 1 21 5 17 9" /><path d="M3 11V9a4 4 0 014-4h14" /><polyline points="7 23 3 19 7 15" /><path d="M21 13v2a4 4 0 01-4 4H3" /></svg>}
        />
        <StatCard
          label="Avg Duration"
          ...
          sub="per call"
          icon={<svg width="20" height="20" className="text-sky-600 dark:text-sky-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>}
        />
        <StatCard
          label="Avg Cost"
          ...
          sub="per call"
          icon={<svg width="20" height="20" className="text-slate-600 dark:text-slate-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M8 8h8M8 12h8M8 16h5" /></svg>}
        />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="xl:col-span-2">
          <CardHeader
            ...
            action={<span className="rounded-md bg-cyan-50 px-2.5 py-1 text-xs font-semibold text-cyan-700 dark:bg-cyan-500/15 dark:text-cyan-200">{calls.length} total calls</span>}
          />
