...
import { fetchDoctors, fetchSettings, updateDoctor, type DoctorRecord, type SettingsRecord } from "../../../api";
...
export function DirectoryPage() {
  ...
  const load = useCallback(async () => {
    if (!token) return;
    try {
      const [d, s] = await Promise.all([fetchDoctors(token), fetchSettings(token)]);
      setDoctors(d);
      setSettings(s);
    } catch (e: unknown) {
